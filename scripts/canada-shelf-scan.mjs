...
const SP='https://sellingpartnerapi-na.amazon.com', CA='A2EUQ1WTGCTBG2';
...
const TERMS=['retractable phone tether','phone tether','anti theft phone strap','phone lanyard retractable'];
const found=new Map();
...
console.log(`\n${found.size} distinct ASINs on the Canadian shelf. Pulling buy-box prices...\n`);
const rows=[];
...
console.log('  CAD    offers  brand              title');
...
if(p.length) console.log(`\nn=${p.length}  min ${p[0].toFixed(2)}  median ${p[Math.floor(p.length/2)].toFixed(2)}  max ${p[p.length-1].toFixed(2)}  mean ${(p.reduce((a,b)=>a+b,0)/p.length).toFixed(2)}  (CAD)`);
