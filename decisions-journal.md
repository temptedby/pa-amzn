...
## 2026-06-30 (cycle 15) — Ship-plan tool: make the merge backlog regenerable (the validated #1 bottleneck)

**Context.** The repeatedly-validated meta-finding (branch-merge-readiness-2026-06-28; every cycle's journal since) is that the bottleneck is SHIPPING, not building: branches pile up, ~none merge. That map was a hand-written one-off — and it has already gone stale: **29 branches on 2026-06-28 -> 69 unmerged now**. Every autonomous lane (ad-engine fixes, pricing, finance, CS, titles, returns) is fully built and parked behind that one merge pass. Building feature branch #70 has near-zero marginal value; the highest-leverage UNBLOCKED item is a TOOL that keeps the merge map current on demand. New independent file set (no ad-engine churn) — so it doesn't add to the very pile it maps.

**Decision.** Built `src/lib/ship-plan.ts` (pure, 9 tests) + read-only `scripts/ship-plan.mjs` (git plumbing only). The lib: `classifyConflict` (splits a trial-merge's conflicting files into trivial append-docs vs real code -> clean/doc-only/code), `collapseStacks` (a branch whose HEAD is contained in another's history is subsumed; emit one unit per tip), `codeFileOverlap` (code files >1 unit touches = cross-unit collision risk). The script trial-merges each tip into main via `git merge-tree --write-tree --name-only` (Git 2.38+ real-merge, exit 1 = conflict), classifies, and emits a tiered ship-plan. Deliverable `confabulator/ship-plan-2026-06-30.md`.

**The validated output.** 69 unmerged branches collapse to **35 mergeable units**: **7 trial-merge clean**, 21 doc-only (trivial journal re-append), **7 real code conflict**, with **30 code files touched by >1 unit**. It empirically reconfirms the 2026-06-28 hand-finding without trusting it: the two ad-engine lineages (`feat/ad-engine-visibility-floor` + `fix/ad-engine-loop-stability-bundle`, plus `bid-stability-gate`) collide on `src/lib/amazon/ad-engine.ts` AND `bid-damping.ts` -> "land one, rebase the rest." It also surfaces NEW collisions the static doc never had: the title/returns/price-bandit feature family fights across 10+ `title-*`/`listings`/`price-bandit` files, and `vercel.json` collides between `sales-anomaly-tripwire` and `weekly-business-report` (two cron registrations).

**Reasoning / RBB.** Trial-merge via `git merge-tree --write-tree` is the documented non-destructive way to test a merge without touching the working tree (Git docs); same primitive the prior audit used by hand, now scripted and exit-code-driven. The trivial-vs-code split is the key insight that makes the report actionable: 21 of 28 "conflicts" are just append-files (decisions-journal, .agent/*) that every branch edits — without separating them, the backlog looks far more tangled than it is. Pure logic is unit-tested; git I/O is a thin shell (same script<->lib split as margin-model<->pricing).

**Industry source.** `git merge-tree --write-tree` real-merge + conflict exit code (git-scm docs, Git 2.38 release notes); `git merge-base --is-ancestor` for ancestry (git-scm). No external vendor claims.

**Trade-offs / rollback.** Read-only + branch-only (git reads only; no checkout/merge/push). A few entries land in "code conflict" for `.gitignore`/`vercel.json`/a `confabulator/*.md` — these ARE real merge points (cron + ignore-rule lines), so flagging them is correct-conservative, not a false positive. The tool maps; it does not merge — the actual review+merge pass is still William's call. Rollback = drop the branch. Honest limit: "clean against main" is per-unit; two units can each be clean yet collide with each other, which is exactly what the collision matrix is for.

**Status.** Built + live-validated on branch `chore/ship-plan-tool-2026-06-30` (off main, standalone, NOT pushed/merged). 76/76 green, tsc clean. **Needs William:** this is the regenerable version of the merge pass that keeps being deferred — run a review+merge session against `ship-plan-2026-06-30.md` (start Tier 1 clean units, then the ad-engine lineage decision). Regenerate any time with `node scripts/ship-plan.mjs`.

## 2026-06-30 (cycle 16) — Merge runbook: turn the ship-plan map into an executable, test-gated session plan

**Context.** The validated #1 bottleneck is SHIPPING, not building (branch-merge-readiness-2026-06-28; reconfirmed every cycle): now **70 unmerged branches**, ~none merged. Cycle 15 built the regenerable ship-plan tool, but it stops at the MAP (tiers + collision matrix). William still has to hand-design the merge ORDER + exact commands each session — and the prior hand-written `merge-execution-sheet-2026-06-28.md` went STALE, the exact failure mode. The missing artifact is the actionable one: the ordered, copy-paste, test-gated merge command sequence. Highest-leverage UNBLOCKED item: it attacks the bottleneck directly, sits on the same already-tested ship-plan file set (touches NO ad-engine.ts, adds nothing to a conflict family), and is regenerable so it can't go stale.

**Decision.** Added a pure, exported `buildMergeRunbook(units, overlap)` to `src/lib/ship-plan.ts` + 5 tests, and wired it into `scripts/ship-plan.mjs` to also emit `confabulator/merge-runbook-<date>.sh` and a runbook section in the ship-plan MD. The function: (Phase 0) land the `gitattributes-union-merge` driver FIRST so doc-only journal/TASKS conflicts auto-resolve; (Phase 1) auto-merge clean + collision-free units, each gated by `npm test`; (Phase 2) auto-merge doc-only + collision-free units (safe once the union driver is in); (Phase 3) everything that collides on a code file becomes a human "land one, rebase the rest" cluster via connected-components of the collision graph, recommending the most-ahead tip to land first. Collisions are NEVER put in the executable steps — only independent units are auto-merged.

**The validated output.** 70 unmerged branches -> **17 auto-merges** (1 union driver + 3 clean + 13 doc-only, each test-gated) + **4 manual clusters**: the ad-engine/pricing family (8 tips on ad-engine.ts/bid-damping.ts/pricing.ts -> land visibility-floor first), the title/listings/returns/finance family (9 tips, chained by bridge files .gitignore + vercel.json), the listing-copy pair, and the review-coverage pair. The `.sh` carries `set -euo pipefail`, `git checkout main`, a per-merge `npm test` halt, and NEVER pushes.

**Reasoning / RBB.** `git merge --no-ff` for reviewable merge commits + a test gate per merge is the documented merge-train / merge-queue discipline (GitHub merge queue; GitLab merge trains): integrate one change, run tests, proceed. The map already trial-merges via `git merge-tree --write-tree` (cycle 15); this converts that classification into the action. Auto-steps are restricted to genuinely-independent units (clean/doc-only AND zero code-collision), so a blind run can only ever touch non-conflicting branches; every real lineage decision is left explicit and human. Pure logic is unit-tested (connected-component grouping, recommend-first ordering, union-driver-first, "colliding units never auto-merge"); git I/O stays a thin shell — same script<->lib split as margin-model and the cycle-15 tool.

**Industry source.** `git merge --no-ff` (git-scm docs); GitHub merge queue / GitLab merge trains (test-gated serial integration); `git merge-tree --write-tree` trial-merge (Git 2.38 release notes) reused from cycle 15. No external vendor claims.

**Trade-offs / rollback.** The generator is read-only (git reads only); the OUTPUT is a script WILLIAM chooses to run — it merges to LOCAL main only and never pushes, the same generate-vs-execute boundary as the prior merge-execution-sheet. Honest limit: Phase-3 connected components are correct but COARSE — two bridge files (.gitignore, vercel.json) chain the title/returns/finance sub-families into one 9-tip cluster, so a human still picks sub-order within a cluster. The recommend-first heuristic (most-ahead, then oldest) guides; it does not decide the ad-engine lineage pick. Branch-only; rollback = drop the branch.

**Status.** Built + live-validated on branch `chore/merge-runbook-2026-06-30` (off the cycle-15 ship-plan branch; NOT pushed/merged). Full suite **81/81 green**, tsc + eslint clean. **Needs William:** run a merge session — `bash confabulator/merge-runbook-2026-06-30.sh` lands the 17 independent units test-gated (regenerate first with `node scripts/ship-plan.mjs`), then make the 4 lineage calls in Phase 3 (start with the ad-engine `visibility-floor`-first decision).

## 2026-06-30 (cycle 19) — Fixed the merge-runbook generator's Phase-0 halt (validated last cycle)

**Context.** Cycle 18 (integration-clean-units-2026-06-30) drove the 17-unit consolidated merge and found a real bug in the runbook GENERATOR (`buildMergeRunbook`, `src/lib/ship-plan.ts`, branch `chore/merge-runbook-2026-06-30`): its Phase-0 emits `git merge --no-ff <union-driver>` as the very first step, but git's `merge=union` driver isn't active until `.gitattributes` is in the working tree. So a fresh run halts on the first merge — the union-driver branch's own journal/TASKS appends conflict before the driver that would auto-resolve them is live. Cycle 18 worked around it by hand and parked the generator fix as "a one-line tweak." This autonomous cycle picked it: the #1 validated bottleneck is SHIPPING (branch-merge-readiness-2026-06-28), every feature lane is already built, so fixing the tool that makes the merge runnable beats building branch #78.

**Options.** (1) Leave the generator, keep hand-fixing each run — rejected: the bug recurs every time anyone runs the runbook fresh, and a hand step defeats a "regenerable plan." (2) Have Phase-0 `git merge` the driver but add a post-hoc conflict resolver — rejected: fights git instead of using the documented driver-activation order. (3) Prepend a Phase-0 PREP step that copies `.gitattributes` out of the driver branch and commits it BEFORE any merge, then still merge the driver branch (now auto-resolving) — chosen: it's exactly the sequence empirically validated in cycle 18 (17/17 merges then auto-resolved), and uses git's own documented mechanism.

**Decision.** In `src/lib/ship-plan.ts`: (a) widened `RunbookStep.kind` to `"prep" | "merge" | "test"`; (b) Phase-0 now emits, in order, a `prep` step (`git checkout <driver> -- .gitattributes && git add .gitattributes && (git diff --cached --quiet || git commit -m "...")` — the `||` guard keeps it `set -euo pipefail`-safe and idempotent if .gitattributes is already committed), then the existing `git merge --no-ff <driver>`, then the `npm test` gate. No union driver found -> no prep step (unchanged). The renderer (`scripts/ship-plan.mjs`) iterates `autoSteps` and prints `s.command` for any kind, so the prep line renders into both the `.sh` and the markdown with no renderer change; the auto-merge count (`kind === "merge"`) is unaffected. Added 2 tests: prep precedes merge precedes test in Phase-0 (`["prep","merge","test"]`) with the exact .gitattributes commands, and NO prep step when there's no union-driver branch.

**Reasoning / RBB.** The fix is the cycle-18 empirical finding turned into code — not a guess. Git's built-in `merge=union` driver only applies to paths once `.gitattributes` declaring it is present in the tree (gitattributes docs); committing it first is the documented activation order. `set -euo pipefail` would abort on a no-op `git commit`, hence the `git diff --cached --quiet ||` guard.

**Industry source.** git built-in `merge=union` driver + gitattributes path-matching semantics (gitattributes(5)); `git merge --no-ff` + test-gate merge-train discipline (GitHub merge queue / GitLab merge trains). Empirical basis: `confabulator/integration-clean-units-2026-06-30.md` (17/17 clean once .gitattributes landed first).

**Trade-offs / rollback.** Phase-0 now has 3 steps instead of 2; the extra prep commit is a tiny, regenerable, local-only commit (the runbook never pushes). If `.gitattributes` is already on main, the guard makes the commit a no-op rather than an error. Branch only, not merged; rollback = revert/delete the branch. Honest limit: I did not re-run the full live `scripts/ship-plan.mjs` against all 78 branches this cycle (it queries live git for every branch and is slow); the unit tests pin the generator's output and the renderer is a pure pass-through of `s.command`.

**Status.** Built + tested on branch `fix/merge-runbook-gitattributes-phase0-2026-06-30` (off `chore/merge-runbook-2026-06-30`; NOT pushed/merged). Full suite **83/83 green**, tsc + eslint clean. **Needs William:** this generator fix should ride along when the merge-runbook tooling lands; once it's in, a fresh `node scripts/ship-plan.mjs` run produces a runbook that no longer halts on its first merge.
