...
### 2026-06-29 (autonomous) — Profitable-store gap analysis (foundation item B), cited

- **Context.** ~30 parked asks in `.agent/questions-for-William.md` + a ~90% sales drop, but no EXTERNAL benchmark of how a profitable Amazon store operates, so the backlog can't be ranked by impact. Foundation-research queue item B was undone. Charter priority #1 = stabilize the drop.
- **Options.** (a) 4th ad-engine audit [rejected — duplicate, William hasn't acted on 3]; (b) build a fix [rejected — TASKS.md is research/audit-only until approval]; (c) **cited profitable-store → gap analysis that collapses the 30 asks into a critical path [chosen]** — new, unblocked, read-only, a real time-saver.
- **Decision.** Wrote `confabulator/profitable-store-gap-analysis-2026-06-29.md`: imported cited 2025-26 benchmarks (CVR, A+ lift, TACoS/ACoS, star-rating cliff, review rate, price band), scored Phone Assured against each from our OWN validated data, ranked 5 gaps, and reduced the backlog to a 3-input + 5-step critical path.
- **Reasoning.** Key external finding: industry says **sub-$15 price makes profit nearly impossible** — our $9.49 hero IS the structural constraint (why break-even ACoS is 52% vs the healthy 15-20%), so recovery must be won on CVR + organic + ad efficiency, never on bidding harder. Biggest CLOSEABLE gap = **zero A+ content** (Amazon's own +3-10%/+15-20% lift); drafts already exist. Confirms bid is the wrong lever (matches the ad-engine R1 finding).
- **Industry source.** Amazon's own A+ CVR figures (via mybrandgenius/xena); FBA price-floor + TACoS benchmarks (ecomdelivery, novadata, sarasanalytics, keywords.am); star-rating/review benchmarks (tracefuse, cantech, sellermetrics, redstag) — all flagged aggregator vs primary in the doc.
- **Trade-offs / status.** Aggregator figures are directional; the two load-bearing ones (A+ lift, $15 floor) are Amazon-sourced / ≥2-source corroborated. GAP 1 (price) deliberately deferred — don't fix margin mid-collapse. DONE on branch `research/profitable-store-gap-analysis-2026-06-29` (off main, NOT pushed/merged). Read-only; no live change, no spend, no send. Rollback = delete branch.

### 2026-06-29 — ES reset/recovery + ad spend-bleed diagnosed
**Context.** William: ES removed too many emails; also spend down + ACOS up each month, ensure keywords are being added.
**Decision.** Stopped aggressive auto-trash (alibaba/marketing rule trashed legit mail); recovered last ~5 days (archived 21 noise, kept 4 genuine, never deleted); new ES = draft + keep-in-inbox, never trash. Confirmed Good Standing entity/file/agent; William ordered the $50 short-form on the DE portal himself (saved ~$200-300); agent declined warmly (draft). Diagnosed ad spend-bleed: TARGET_ACOS is 0.50 (the alert's "30%" is a stale string); root cause is broken harvest (H1) adding only 2 kw/month. C1 + H1 fixes built/tested on branches, awaiting merge/deploy approval.
**Industry source.** DE Division of Corporations cert fees ($50 short / $175 long); Amazon Ads v3 reporting; getMyFeesEstimate break-even 52%.
**Trade-offs / lesson.** Over-restored 25 on first recovery pass (noise filter too narrow) -> corrected to archive 21 / keep 4. MoM trend report flaky -> did NOT present fabricated numbers (RBB). Live cron + deploy remain gated; fixes wait for William.
**Status.** Inbox recovered + clean; Good Standing ordered by William; ad fixes ready to deploy on approval. All on branch bear/24-7-setup-2026-06-25.

### 2026-06-29 (autonomous) — Validated-data baseline = one source of truth (foundation item C)

- **Context.** Numbers (8,134u/$127,456, COGS, fees, ad recovery, inventory) were scattered across the charter, journal entries, and one-off reports, and the 2026-06-24 PM lesson logged ~an hour lost to reactive probes that the documented limits would have pre-empted. Foundation-queue item C (validated-data baseline) was undone. Charter priority = stabilize the drop, which needs one trusted source of truth.
- **Options.** (a) 4th ad-engine audit [rejected — duplicate; William hasn't acted on 3]; (b) build an ad/price fix [rejected — TASKS.md is research/audit-only until approval, and the merge backlog is the bottleneck]; (c) **consolidate every validated number with provenance into one doc + a read-only regenerator script [chosen]** — unblocked, read-only, a genuine time-saver that prevents re-derivation.
- **Decision.** Built `scripts/baseline.mjs` (read-only Turso pull: table counts, inventory by SKU, ad-engine/review recency; column-name-resilient; reruns safely) and `confabulator/validated-data-baseline-2026-06-29.md` (identity/structure, unit economics, authoritative sales figures, the live DB baseline, the data-access reality table, and how to keep it current). Every figure carries source + retrieval method + known limit.
- **Reasoning / new findings from the live pull.** (1) The decline is a **12-month bleed, not a cliff**: monthly units ~492 (2025-06) → 174 (2026-05) → 62 partial June; YoY June ≈ −83%. That fits CVR/rating/organic erosion, reinforcing "fix conversion, not bids." (2) **Only 4 of 13 SKUs have FBA stock** — the Single hero + 3 core black packs are fine, but the Pro variant B0CFYVNBJX, Assured B0CFYW3NYQ, all 3 white packs, and the Securisee lanyard read zero FBA; variant stockouts are a plausible *contributor* on top of CVR. (3) review_requests: 120 backfilled 2026-06-09 then a thin trickle, nothing after 06-25 — corroborates the coverage leak. (4) Confirmed the 0-count ad tables are expected (engine reads the API live), not the drop cause.
- **Industry source.** Internal validated data only (SP-API all-orders report, Turso DB read, William-confirmed console/COGS figures, getMyFeesEstimate); no aggregator numbers in this baseline by design. Data-access limits cited from prior journal research (Amazon ~2-yr order retention; Ads API ~60-95d lookback; Brand-Analytics 403).
- **Trade-offs / status.** §3 lifetime figures stay console-held (don't re-probe the API). DONE on branch `research/validated-data-baseline-2026-06-29` (off main, NOT pushed/merged). Read-only; no live change, no spend, no send. Rollback = delete branch. NEEDS WILLIAM: confirm whether the 9 zero-FBA SKUs are intentional (discontinued) or lapsed restocks.
